...
## Чтения идут из зеркала, записи — в Таблицу (Фаза 1, 2026-08-18)
Повод: в проде курьер трижды подряд получил пустую биржу, потому что Google
отдавал `RemoteDisconnected`/`503` (видно в логах за 17.08). Теперь **чтения
сначала пробуют SQLite-зеркало**, и сбой Google перестал быть виден пользователю.

Переведены (в каждой — `if db.is_fresh(): try: return db.<...>` и падение
обратно в Таблицу при промахе/исключении):
`_sync_get_free_orders`, `_sync_get_driver`, `_sync_get_all_active_drivers`,
`_sync_get_orders_for_dashboard`, `_sync_get_drivers_for_dashboard`
(driver_bot), `_sync_get_client_order_statuses` (client_bot).

**Запись НЕ трогали — источник истины остаётся "Лист1".** `_sync_take_order`
и соседи по-прежнему перечитывают статус из Таблицы под `_order_take_lock`,
поэтому устаревшая карточка в биржe даёт максимум «заказ уже забрали», а не
двойной захват. Это ключевая гарантия — не ломать её.

**Write-through**: после КАЖДОЙ успешной записи в Таблицу зовём зеркало, иначе
оно отстаёт на целый интервал снапшота — `db.mark_order_taken` /
`mark_order_released` / `mark_order_status` / `mark_driver_status` /
`upsert_order_from_row`. Добавляешь новую точку записи — добавь и вызов сюда
(это третье место после "Лист1" и листа "Заказы", см. раздел про дубли статуса).
Где заказа в зеркале может ещё не быть (создание клиентом, «отдать в работу»
у менеджера) — только `upsert_order_from_row(row)`, `UPDATE` там промахнётся.

**Снапшот 600с → 60с** (`SNAPSHOT_INTERVAL_SEC` в .env). Нагрузка на Sheets при
этом упала, а не выросла: биржа опрашивалась каждые 15с полным
`get_all_values()` (240 чтений/час) — теперь это SQLite.

**Доступ курьера НЕ обслуживается общим кэшом.** Гейт один —
`_get_active_driver()` (`driver_data[0] == "ACTIVE"`), и он читает через
`_sync_get_driver(chat_id, for_auth=True)` → `db.is_auth_fresh()`: порог
`AUTH_MAX_AGE_SEC` = 180с (три интервала снапшота) и сверка по листу
«Водители», а не по orders. Причина: отзыв доступа у АКТИВНОГО курьера
делается правкой листа руками, write-through этот путь не ловит — с общими
900с уволенный курьер работал бы до 15 минут, и как раз при сбоях Google.
Протухло → идём в Таблицу; Таблица недоступна → доступа нет (fail-closed, как
и было до Фазы 1). Тот же `for_auth=True` у `build_driver_main_menu` и
`cmd_start_driver`. **Появится новая проверка прав по статусу — читай с
`for_auth=True`.** Остальные вызовы (язык, ФИО для подписи) — обычный путь.

Нюансы:
- `db.is_fresh(max_age_sec, table)` — зеркало старше 15 мин считается
  протухшим, чтение уходит в Таблицу. Пустая таблица → тоже «не свежо»
  (холодный старт). `table` только из белого списка (имя таблицы идёт в SQL
  как идентификатор, плейсхолдером его не подставить).
- `row_num`/`row` из зеркала = `0`: номеров строк там нет и они не нужны
  (действия идут по ID).
- `db._price_str` отдаёт **пустую строку** для нуля. Не «чинить» на `'0'`:
  пустая ставка в Таблице означает «ставка по умолчанию», а `'0'` пройдёт
  проверку `if row[4]` и обнулит курьеру заработок в отчёте.
- Схема зеркала догоняется в `db._migrate()` (PRAGMA + ALTER): volume
  `/data/mavsim.db` переживает пересборку, `CREATE TABLE IF NOT EXISTS` новые
  столбцы не добавит. Добавляешь поле — добавь и в `_migrate`.

## Панель менеджера обновляется сама (2026-08-18)
Кнопки "🔄 Обновить" больше нет. Панель пересылается заново (со стиранием
предыдущего сообщения — как declutter в других ботах) при действиях менеджера
и фоновым циклом `_panel_autorefresh_loop`, если данные реально изменились.

**Почему не инлайн-кнопка и не редактирование сообщения:** данные панели едут
внутри URL WebApp-кнопки (`?d=<base64>`), а у отправленного сообщения URL уже
не поменять — значит «освежить» можно только новым сообщением. Перевести вход
на инлайн-кнопку (её разметку редактировать МОЖНО) нельзя: Telegram отдаёт
`Telegram.WebApp.sendData()` обратно боту **только** для WebApp, запущенных с
reply-клавиатуры. А на `sendData` держатся все действия панели — set_ready,
change_status, reassign, block_driver, отчёты. Не переписывай на инлайн, не
проверив это ограничение.

Настоящее живое обновление (пока панель ОТКРЫТА) без бэкенда невозможно в
принципе — это Фаза 2 (FastAPI + туннель, страница на fetch+polling).

## Блокировка курьера (2026-08-18)
Кнопка «Блок»/«Вернуть» в списке курьеров панели → `block_driver` /
`unblock_driver` → `manager_bot._sync_set_driver_access` (статус `BLOCKED`,
write-through `db.mark_driver_status`). Блокировать можно только `ACTIVE`,
разблокировать только `BLOCKED`. Заблокированные приходят в payload панели
(иначе их нечем вернуть), но исключены из списка переназначения и из счётчика
«курьеров в работе». Курьер получает уведомление на своём языке; ветка
"blocked" в `cmd_start_driver` ловит `BLOCKED` без правок.
**Отзыв доступа делай этой кнопкой, а не правкой листа руками** — правка мимо
кода не даёт write-through, и доступ пропадёт только со следующим снапшотом.
...
| — | _panel_view — текст + ссылки + подпись содержимого (подпись нужна, чтобы не слать панель, когда данные не менялись) |
| — | _send_panel (reply keyboard: "🎛 Открыть панель" + "📊 Статистика"; шлёт новую панель и УДАЛЯЕТ предыдущую) |
| — | _panel_autorefresh_loop — панель обновляется САМА раз в PANEL_AUTOREFRESH_SECONDS (60с), но только если данные изменились И менеджер что-то делал за последние PANEL_ACTIVE_WINDOW_SECONDS (900с). Активность отмечает _touch_panel из _is_manager |
| — | panel_refresh_text — оставлен только для старых клавиатур с кнопкой "🔄 Обновить"; самой кнопки в новой панели нет |
| 380 | order_accept (oa:) |
